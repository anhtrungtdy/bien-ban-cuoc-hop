...
// Helper to get all keys parsed from env
...
// Check if keys exist (used for UI checks)
...
// This function tries to execute the operation with available keys one by one until success
const withGeminiRetry = async <T>(
  ...
  // Shuffle keys to distribute load, avoiding hotspots on the first key
  ...
  for (const apiKey of shuffledKeys) {
    try {
      ...
      // console.log(`Attempting ${operationName} with key ...${apiKey.slice(-4)}`);
      return await operation(ai);
    } catch (error: any) {
      console.warn(`[Gemini Service] Attempt failed with key ...${apiKey.slice(-4)} for ${operationName}. Error:`, error);
      lastError = error;
      // Continue to the next key in the loop
    }
  }
  
  // If all keys fail, throw the last error encountered
  console.error(`[Gemini Service] All keys failed for ${operationName}.`);
  throw lastError;
};
...
export const identifyParticipants = async (
  ...
  return withGeminiRetry("identifyParticipants", async (ai) => {
    ...
    try {
        ...
        console.warn("Failed to parse JSON in identifyParticipants, returning empty.");
        return [];
    }
  });
};
...
export const extractRawMeetingData = async (
  ...
  return withGeminiRetry("extractRawMeetingData", async (ai) => {
    const modelName = 'gemini-3-flash-preview'; 
...
export const mapContentToTemplate = async (
  ...
  return withGeminiRetry("mapContentToTemplate", async (ai) => {
    const modelName = 'gemini-3-flash-preview'; 
...
export const generateFinalMinutes = async (
    ...
    return withGeminiRetry("generateFinalMinutes", async (ai) => {
        const modelName = 'gemini-3-flash-preview';
